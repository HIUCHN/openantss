/*
  # Fix Avatar Storage with Proper Permissions

  1. Changes
    - Create a storage bucket for avatars using the storage API
    - Set up proper folder-based organization for user avatars
    - Create RLS policies that allow users to manage their own avatars
    - Allow public access to avatars of users with public profiles

  2. Security
    - Users can only upload/update/delete their own avatars
    - Public access is limited to avatars of users with public profiles
    - Private avatars are only accessible to their owners

  3. Organization
    - Each user's avatars are stored in their own folder using user ID
    - Simplifies permissions and improves organization
*/
